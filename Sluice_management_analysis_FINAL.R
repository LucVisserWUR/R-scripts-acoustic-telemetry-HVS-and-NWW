title: "Getij, Spui Kier analysis at HVS dam improved version that allows for Spui/Kier transition hours " 
Author: "Luc Visser"
date: "2024-04-16"
...
#rm(list = ls())
#rm(list = setdiff(ls(), "Detections2023_2024"))
knitr::opts_chunk$set(echo = TRUE)

# Load dataset with getij, spui and kier data
Getij_Spui_Kier_data <- read.csv("Env_Getij_Spui_Kier_HVS.csv")

# To set every DateTime value in a correct format, Spui and Kier data needs some alternations to merge them with the relevant date

# Rename column names of GetijVan and GetijTot
...
# Order this dataset to make it easy to see how date will be merged with time (HH:MM)
...
# Create 2 subsets: one in which it can be assumed that the date equals the GetijVan date and one where you check the data in which you deal with date issue Spui and Kier data
...
# Create dataframes with just the relevant time bins in there. Starting and end times are taken from the Environmental_Variables_HVS_NWW file
...
# Create a Getij_Spui_Kier_data_UTC_total_final that allows for Spui and Kier moments happening in the same hour: these are called 'transition' period
