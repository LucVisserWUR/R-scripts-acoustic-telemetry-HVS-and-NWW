title: "Environmental factor ~ DE correlation HVS and NWW telemetry networks: Main script behind plots" 
Author: "Luc Visser"
date: "2024-20-04"
...
# 2. Select correct columns for waterheight, chloride and temperature
...
# 10. Clarify the total env. variable dataset, the HVS river, HVS sea and NWW Env.Var. - DE datasets and select receiver combinations 
...
Waterheight_NWW <- read.csv("Env_waterhoogte_hoekvholland_0308.csv") # TAKE WATERHEIGHT HOEK VAN HOLLAND
...
Transmitter_information <- read.csv("Transmitterinformation2023_2024.csv") # In case you need to know what the power output of certain transmitters is 
...
# Merge discharge datasets together to create a dataset for the complete time period with all available data (2016 - 2023)

# THIS CODE DOES NOT WORK PROPERLY ANYMORE. FIND OUT HOW. 
...
# --> Chapter 1  of Env_Variables_HVS_NWW_Plots for visualisation


## 2. SELECT CORRECT COLUMNS FOR WATERHEIGHT, CHLORIDE AND TEMPERATURE

# Waterheight, chloride and temperature were provided in G (validated) and O (unvalidated) columns. Select only the G columns.    

# WATERHEIGHT. !! YOU TOOK O VALUES FOR NOW !!
...
# Create total HVS waterheight dataset & calculate delta waterheight
...
# CHLORIDE. Select only 200cm depth. Although chloride differs sign. over depth, 200cm was the most relevant depth with respect to receiver positioning
...
# Calculate the total sluiceopening (cm) by adding up sluice opening HVS sluise 1-17
Hefhoogte_relevant$Total_sluice_opening_HVS_cm <- rowSums(Hefhoogte_relevant[, 2:18])

# Select only relevant columns for hefhoogte relevant file
Hefhoogte_relevant <- Hefhoogte_relevant %>%
...
# Check if there are any NA values in the chloride columns after selecting only validated values in Chapter 2
NA_count_Chloride_HVS_river <- sum(is.na(Chloride_HVS_relevant$Chloride_stelldbnn_200cm))  
print(NA_count_Chloride_HVS_river) # 133 / 52398 = 0.2% vs 0 NA when O and G were merged. Means that 133 O values of Chloride_stelldbnn_200cm were used to make dataset complete
NA_count_Chloride_HVS_sea <- sum(is.na(Chloride_HVS_relevant$Chloride_stelldbtn_200cm))  
print(NA_count_Chloride_HVS_sea) # 3618 / 52398 = 6.9% vs 0 NA when O and G were merged. Means that 3618 O . values of Chloride_stelldbtn_200cm were used to make dataset complete
...
# Check if there are any NA values in the Temperature columns after selecting only validated values in Chapter 2
NA_count_Temp_HVS_river <- sum(is.na(Temperature_HVS_relevant$Temp_stelldbnn_200cm))  
print(NA_count_Temp_HVS_river) #  133 / 52398 = 0.2% vs 133 NA when O and G were merged. Means that no O values of Chloride_stelldbnn_200cm were used to make dataset complete
NA_count_Temp_HVS_sea <- sum(is.na(Temperature_HVS_relevant$Temp_stelldbtn_200cm))  
print(NA_count_Temp_HVS_sea) # 10 / 52398 = 0.01% vs 10 NA when O and G were merged. Means that no . values of Chloride_stelldbtn_200cm were used to make dataset complete
...
# Rename DateTime column names to make the imported csv file mergible with other files
...
# *2 Moreover, it seems like there is no winter - summer time transition judging by 02:00:00 March 26 2023, when that transition would have taken place) 
...
Tilt_Noise_Temp_HDC_relevant <- Tilt_Noise_Temp_HDC_relevant[, !(names(Tilt_Noise_Temp_HDC_relevant) %in% c('Units'))] # Also remove the Units column: add those in the desciption column names
...
Tilt_Noise_Temp_HS8_relevant <- Tilt_Noise_Temp_HS8_relevant[, !(names(Tilt_Noise_Temp_HS8_relevant) %in% c('Units'))] # Also remove the Units column: add those in the desciption column names
...
# Reshape the dataframe to get tilt, environmental noise and temperature as column names
Tilt_Noise_Temp_HDC_relevant <- Tilt_Noise_Temp_HDC_relevant %>% 
...
# Reshaping succesful! Now rename the columns, including the receiver name
...
# First discharge with Waterheight_NWW
...
# --> Chapter 2  of Env_Variables_HVS_NWW_Plots for visualization

## 7. CREATE 1 HOURLY BINS FOR THE ENV. VARIABLES. 
relevant_time_min_utc_1hour <- as.POSIXct("2023-01-26 22:59:00", tz = "UTC") # Take time points to get the exact time period of one_hourly_observed_expected_values_NWW
...
Enviro_combined_hours <- mutate(Enviro_combined_hours, Hour = format(DateTime_UTC, "%Y-%m-%d %H:00:00")) # Create the 1 hour bins. hours stands for 1 hour bins

Enviro_combined_hours_average <- Enviro_combined_hours %>% # Calculate the averages
  dplyr::group_by(Hour) %>%
  dplyr::summarise(Avg_Discharge_HVS_m3_s = mean(Discharge_HVS_m3_s, na.rm = TRUE),
            ...
            #Avg_Chloride_stelldbnn_600cm = mean(Chloride_stelldbnn_600cm, na.rm = TRUE),
            #Avg_Chloride_stelldbnn_1100cm = mean(Chloride_stelldbnn_1100cm, na.rm = TRUE),
            Avg_Chloride_stelldbtn_200cm = mean(Chloride_stelldbtn_200cm, na.rm = TRUE),
            #Avg_Chloride_stelldbtn_500cm = mean(Chloride_stelldbtn_500cm, na.rm = TRUE),
...
Enviro_combined_hours_average$Avg_Discharge_NWW_m3_s <- round(Enviro_combined_hours_average$Avg_Discharge_NWW_m3_s, digits = 1)# Round average values
...
#Enviro_combined_hours_average$Avg_Chloride_stelldbnn_600cm <- round(Enviro_combined_hours_average$Avg_Chloride_stelldbnn_600cm, digits = 1)
#Enviro_combined_hours_average$Avg_Chloride_stelldbnn_1100cm <- round(Enviro_combined_hours_average$Avg_Chloride_stelldbnn_1100cm, digits = 1)
...
#Enviro_combined_hours_average$Avg_Chloride_stelldbtn_500cm <- round(Enviro_combined_hours_average$Avg_Chloride_stelldbtn_500cm, digits = 1)
...
# Calculate the absolute tilt (provided - 180)
...
#Enviro_combined_hours_average_till_December2023$Avg_Chloride_stelldbnn_600cm <- as.numeric(Enviro_combined_hours_average_till_December2023$Avg_Chloride_stelldbnn_600cm)
...
# --> Chapter 4 of Env_Variables_HVS_NWW_Plots for visualisation
...
#ID_closed_sluice <- which(Enviro_combined_DE_1hour_HVS$Spui_Kier == "Closed") # Check if closed values are inserted
...
# Only use code below if the 'glitch' referred to below actually happened in the dataset. This is not always the case

#if ("2023-01-27" %in% as.character(Enviro_combined_DE_1hour_HVS$Hour)) { # This seems to be a strange glitch that appeared after filtering for relavant DateTime points
#Enviro_combined_DE_1hour_HVS$Hour[Enviro_combined_DE_1hour_HVS$Hour == "2023-01-27"] <- as.POSIXct("2023-01-27 00:00:00", tz = "UTC")
#} # This glitch appeared only for HVS data, not for NWW
...
# Modify the newly created total NWW script a bit. Remove all values where no detections occured
# Enviro_combined_DE_1hour_NWW <- Enviro_combined_DE_1hour_NWW[complete.cases(Enviro_combined_DE_1hour_NWW$obs), ] # remove values where id == NA, these are not linked to observations
...
# Create subsets for relevant two-week time periods (April, July, December 2023)

# HVS river
Env_DE_HDC_HDB_Apr <- subset(Env_DE_HDC_HDB, DateTime_UTC > "2023-04-10 09:00:00" & DateTime_UTC < "2023-04-24 09:00:00" )
Env_DE_HDC_HDB_Jul <- subset(Env_DE_HDC_HDB, DateTime_UTC > "2023-07-10 09:00:00" & DateTime_UTC < "2023-07-24 09:00:00" )
Env_DE_HDC_HDB_Dec <- subset(Env_DE_HDC_HDB, DateTime_UTC > "2023-12-10 09:00:00" & DateTime_UTC < "2023-12-24 09:00:00" )

Env_DE_HDC_HDD_Apr <- subset(Env_DE_HDC_HDD, DateTime_UTC > "2023-04-10 09:00:00" & DateTime_UTC < "2023-04-24 09:00:00" )
Env_DE_HDC_HDD_Jul <- subset(Env_DE_HDC_HDD, DateTime_UTC > "2023-07-10 09:00:00" & DateTime_UTC < "2023-07-24 09:00:00" )
Env_DE_HDC_HDD_Dec <- subset(Env_DE_HDC_HDD, DateTime_UTC > "2023-12-10 09:00:00" & DateTime_UTC < "2023-12-24 09:00:00" )

# HVS sea
Env_DE_HS6_HS8_Apr <- subset(Env_DE_HS6_HS8, DateTime_UTC > "2023-04-10 09:00:00" & DateTime_UTC < "2023-04-24 09:00:00" )
Env_DE_HS6_HS8_Jul <- subset(Env_DE_HS6_HS8, DateTime_UTC > "2023-07-10 09:00:00" & DateTime_UTC < "2023-07-24 09:00:00" )
Env_DE_HS6_HS8_Dec <- subset(Env_DE_HS6_HS8, DateTime_UTC > "2023-12-10 09:00:00" & DateTime_UTC < "2023-12-24 09:00:00" )

# NWW
Env_DE_NW2_NW3_Apr <- subset(Env_DE_NW2_NW3, DateTime_UTC > "2023-04-10 09:00:00" & DateTime_UTC < "2023-04-24 09:00:00" )
Env_DE_NW2_NW3_Jul <- subset(Env_DE_NW2_NW3, DateTime_UTC > "2023-07-10 09:00:00" & DateTime_UTC < "2023-07-24 09:00:00" )
Env_DE_NW2_NW3_Dec <- subset(Env_DE_NW2_NW3, DateTime_UTC > "2023-12-10 09:00:00" & DateTime_UTC < "2023-12-24 09:00:00" )


# --> Chapter 6 of Env_Variables_HVS_NWW_Plots for visualisation
